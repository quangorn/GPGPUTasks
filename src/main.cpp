...
bool findDevice(bool useGpu, cl_platform_id &foundPlatform, cl_device_id &foundDevice) {
    ...
    for (cl_uint platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        for (cl_uint deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            if (!deviceIsFound && ((useGpu && type == CL_DEVICE_TYPE_GPU) || (!useGpu && type == CL_DEVICE_TYPE_CPU))) {
                foundPlatform = platform;
                foundDevice = device;
                deviceIsFound = true;
                std::cout << "        Using this device!!!!" << std::endl;
            }
...
int main() {
    ...
    cl_command_queue queue = clCreateCommandQueue(context, device, queueProperties, &err);
    OCL_SAFE_CALL(err);

    const unsigned int n = 100 * 1024 * 1024;
    const auto dataSize = sizeof(float) * n;
    ...
    // 4 Создайте три буфера в памяти устройства (в случае видеокарты - в видеопамяти - VRAM) - для двух суммируемых массивов as и bs
    // (они read-only) и для массива с результатом cs (он write-only)
    ...
    cl_mem bufferAs = clCreateBuffer(context, CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY, dataSize, as.data(), &err);
    OCL_SAFE_CALL(err);
    cl_mem bufferBs = clCreateBuffer(context, CL_MEM_COPY_HOST_PTR | CL_MEM_READ_ONLY, dataSize, bs.data(), &err);
    OCL_SAFE_CALL(err);
    cl_mem bufferCs = clCreateBuffer(context, CL_MEM_WRITE_ONLY, dataSize, nullptr, &err);
    OCL_SAFE_CALL(err);
    ...
    cl_program program = clCreateProgramWithSource(context, 1, sources.data(), nullptr, &err);
    OCL_SAFE_CALL(err);

    // 8 Теперь скомпилируйте программу и напечатайте в консоль лог компиляции
    // см. clBuildProgram
    auto buildErr = clBuildProgram(program, 1, &device, nullptr, nullptr, nullptr);
    ...
    err = clGetProgramBuildInfo(program, device, CL_PROGRAM_BUILD_LOG, logSize, log.data(), nullptr);
    OCL_SAFE_CALL(err);
    ...
    //   - Дождаться завершения полученного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 50; ++i) {
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(queue, kernel, 1, nullptr, &globalWorkSize, &workGroupSize, 0, nullptr,
                                                 &event));
        ...
        std::cout << "GFlops: " << n / (t.lapAvg() * 1.0e+9) << std::endl;
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << 3 * dataSize / (t.lapAvg() * 1024 * 1024 * 1024) << " GB/s" << std::endl;
    }
    ...
    // 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, bufferCs, true, 0, dataSize, cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
        std::cout << "Result data transfer time: " << t.lapAvg() << "+-" << t.lapStd() << " s" << std::endl;
        std::cout << "VRAM -> RAM bandwidth: " << dataSize / (t.lapAvg() * 1024 * 1024 * 1024) << " GB/s" << std::endl;
    }
